PROGRAM plenum_chamber_simple
	...
	allowable_value = 0.1**4

	OPEN (1, file='output02.dat', status='replace')
...
DO l = 1,max_number_of_iterations
	...
	DO i = 2, mesh_num -1
		...
		extra_correction_e = (sudo_cond_v(i - 1) + sudo_cond_v(i)) / 2 / (sudo_cond_v(i + 1) - sudo_cond_v(i - 1))
		extra_correction_w = (sudo_cond_v(i) + sudo_cond_v(i + 1)) / 2 / (sudo_cond_v(i + 1) - sudo_cond_v(i - 1))
		sudo_v(i) = sudo_v(i) + (sudo_cond_p(i) - sudo_cond_p(i + 1)) * (extra_correction_w - extra_correction_e)  * ave_a
		temp_real = density * (sudo_cond_v(i) + sudo_cond_v(i + 1)) / 2 * cross_sectional_a(i + 1)
		sudo_v(i) = sudo_v(i) / temp_real
		param_d(i) = ave_a / temp_real
	END DO
	...
	PRINT *,sudo_v(1),sudo_v(2),sudo_v(3),sudo_v(4),sudo_v(5)
	!不足緩和
	DO i = 1,mesh_num
	...
	DO i = 2, mesh_num -1
		ave_a = (cross_sectional_a(i + 1) + cross_sectional_a(i)) / 2
		res_v(i) = density * (sudo_cond_v(i - 1) + sudo_cond_v(i)) / 2 * cross_sectional_a(i) * sudo_cond_v(i - 1)
		extra_correction_e = (sudo_cond_v(i - 1) + sudo_cond_v(i)) / 2 / (sudo_cond_v(i + 1) - sudo_cond_v(i - 1))
		extra_correction_w = (sudo_cond_v(i) + sudo_cond_v(i + 1)) / 2 / (sudo_cond_v(i + 1) - sudo_cond_v(i - 1))
		res_v(i) = res_v(i) + (sudo_cond_p(i) - sudo_cond_p(i + 1)) * (extra_correction_w - extra_correction_e)  * ave_a
		temp_real = density * (sudo_cond_v(i) + sudo_cond_v(i + 1)) / 2 * cross_sectional_a(i + 1)
		res_v(i) = temp_real * sudo_cond_v(i) - res_v(i)
